...
static void treeFactory(GenericTree<int>& tree) {
  ...
  tree.createRoot(4);
  auto left = tree.getRootPtr()->addChild(8);
  auto right = tree.getRootPtr()->addChild(15);
  auto leftLeft = left->addChild(16);
  auto leftRight = left->addChild(23);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  std::vector<TreeNode*> childPtr;
  std::vector<TreeNode*> next_childPtr;

  // Initialize childPtr
  childPtr = rootNodePtr->childrenPtrs;

  // child ptr of current node
  while (childPtr.size() != 0) {
    ...
    for (size_t i = 0; i < childPtr.size(); i++) {
      ...
      for (size_t j = 0; j < childPtr[i]->childrenPtrs.size(); j++) {
        next_childPtr.push_back(childPtr[i]->childrenPtrs[j]);
      }
    }
    childPtr.clear();
    childPtr = next_childPtr;
  }
